...
export default function trellisExtension(pi: {
  ...
  // Sub-agent execution
  //
  // Upstream Trellis currently generates a custom `trellis_subagent` tool that
  // shells out to nested `pi` processes. This repository intentionally disables
  // that generated runner and uses the installed `npm:pi-subagents` package
  // instead. Project Trellis agents live in `.pi/agents/` and are discovered by
  // pi-subagents as normal project agents (`trellis-implement`,
  // `trellis-ui-implement`, `trellis-test`, `trellis-check`, `trellis-research`).
  //
  // Keep the rest of this extension loaded for Trellis session context, workflow
  // breadcrumbs, and TRELLIS_CONTEXT_ID propagation, but do not register the
  // generated `trellis_subagent` tool.
  ...
  pi.on?.("input", (event, ctx) => {
    ...
    // Drop session-overview (ov) from per-message injection: it is bulky
    // (git status, recent commits, full task list) and already injected into
    // the system prompt via before_agent_start every turn. Keep only the
    // lightweight workflow-state breadcrumb (wf) per user message.
    const { wf } = getTurnCtx(k);
    const injection = wf;
