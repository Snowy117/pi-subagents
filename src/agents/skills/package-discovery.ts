/**
 * Skill search-path discovery from filesystem, installed packages, and settings.
 */
...
import { SOURCE_PRIORITY, type SkillSearchPath, type SkillSource } from "./types.ts";

export function readOptionalJsonFile(filePath: string, label: string): unknown {
	try {
...
export function readJsonFileBestEffort(filePath: string): unknown {
	try {
		return JSON.parse(fs.readFileSync(filePath, "utf-8"));
	} catch {
...
export function extractSkillPathsFromPackageRoot(packageRoot: string, source: SkillSource, bestEffort = false): SkillSearchPath[] {
...
export let cachedGlobalNpmRoot: string | null = null;
...
export function collectInstalledPackageSkillPaths(cwd: string, agentDir: string): SkillSearchPath[] {
...
export function collectSettingsSkillPaths(cwd: string, agentDir: string): SkillSearchPath[] {
	const results: SkillSearchPath[] = [];
	const projectConfigDir = getProjectConfigDir(cwd);
	const settingsFiles = [
...
export function stripGitRef(repoPath: string): string {
...
export function parseGitPackagePath(source: string): { host: string; repoPath: string } | undefined {
	const spec = source.slice(4).trim();
	if (!spec) return undefined;
...
export function collectSettingsPackageSkillPaths(cwd: string, agentDir: string): SkillSearchPath[] {
	const projectConfigDir = getProjectConfigDir(cwd);
	const settingsFiles = [
...
export function buildSkillPaths(cwd: string, agentDir: string): SkillSearchPath[] {
	const projectConfigDir = getProjectConfigDir(cwd);
