...
describe("builtin agent disabling", () => {
	...
	it("filters a per-agent disabled builtin from runtime discovery while keeping it in discoverAgentsAll", () => {
		...
		const runtimeDelegate = discoverAgents(tempProject, "both").agents.find((agent) => agent.name === "delegate");
		assert.equal(runtimeDelegate, undefined);

		const allDelegate = discoverAgentsAll(tempProject).builtin.find((agent) => agent.name === "delegate");
		assert.ok(allDelegate);
		assert.equal(allDelegate.disabled, true);
		assert.equal(allDelegate.override?.scope, "user");
	});
	...
	it("an explicit user override opts a builtin out of user-scope bulk disable", () => {
		...
		assert.ok(delegate);
		assert.equal(delegate.disabled, undefined);
	...
	it("project bulk disable beats user per-agent re-enable overrides", () => {
		...
		const delegate = discoverAgents(tempProject, "both").agents.find((agent) => agent.name === "delegate");
		assert.equal(delegate, undefined);

		const allDelegate = discoverAgentsAll(tempProject).builtin.find((agent) => agent.name === "delegate");
		assert.ok(allDelegate);
		assert.equal(allDelegate.disabled, true);
		assert.equal(allDelegate.override?.scope, "project");
	});
