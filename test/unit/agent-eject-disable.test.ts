...
describe("agent eject/disable/enable/reset management actions", () => {
	...
	describe("eject", () => {
		it("copies a bundled builtin to user scope verbatim and shadows the builtin", () => {
			...
			const builtin = discoverAgentsAll(tempDir).builtin.find((a) => a.name === "delegate");
			assert.ok(builtin);

			const ejected = handleManagementAction("eject", { agent: "delegate" }, ctx);
			assert.equal(ejected.isError, false);
			assert.match(readText(ejected), /Ejected agent 'delegate' from builtin to user scope/);

			const target = userAgentPath("delegate");
			assert.equal(fs.existsSync(target), true);
			assert.equal(fs.readFileSync(target, "utf-8"), fs.readFileSync(builtin.filePath, "utf-8"));

			const effective = discoverAgents(tempDir, "both").agents.find((a) => a.name === "delegate");
			assert.ok(effective);
		...
		it("ejects to project scope when agentScope is project", () => {
			...
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			const ejected = handleManagementAction("eject", { agent: "delegate", agentScope: "project" }, ctx);
			assert.equal(ejected.isError, false);
			assert.match(readText(ejected), /to project scope/);
			assert.equal(fs.existsSync(projectAgentPath("delegate")), true);
			assert.equal(discoverAgentsAll(tempDir).project.find((a) => a.name === "delegate")?.source, "project");
		});

		it("copies a package agent that shadows a builtin by runtime precedence", () => {
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			writePackageAgent("delegate");
			assert.equal(discoverAgents(tempDir, "both").agents.find((a) => a.name === "delegate")?.source, "package");

			const ejected = handleManagementAction("eject", { agent: "delegate" }, ctx);
			assert.equal(ejected.isError, false);
			assert.match(readText(ejected), /from package to user scope/);
			assert.equal(fs.readFileSync(userAgentPath("delegate"), "utf-8"), fs.readFileSync(packageAgentPath("delegate"), "utf-8"));
		});

		it("refuses invalid management scopes without writing user files", () => {
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			const ejected = handleManagementAction("eject", { agent: "delegate", agentScope: "workspace" }, ctx);
			assert.equal(ejected.isError, true);
			assert.match(readText(ejected), /agentScope must be 'user' or 'project'/);
			assert.equal(fs.existsSync(userAgentPath("delegate")), false);
		});

		it("refuses to eject when a custom agent already exists", () => {
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			fs.mkdirSync(path.dirname(userAgentPath("delegate")), { recursive: true });
			fs.writeFileSync(userAgentPath("delegate"), "---\nname: delegate\ndescription: Mine\n---\n\nMine.\n", "utf-8");
	...
	describe("disable", () => {
		it("hides a builtin from runtime discovery via a user settings override", () => {
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			const disabled = handleManagementAction("disable", { agent: "delegate" }, ctx);
			assert.equal(disabled.isError, false);
			assert.match(readText(disabled), /Disabled agent 'delegate' via user settings override/);

			assert.equal(discoverAgents(tempDir, "both").agents.find((a) => a.name === "delegate"), undefined);
			const all = discoverAgentsAll(tempDir).builtin.find((a) => a.name === "delegate");
			assert.ok(all);
			assert.equal(all.disabled, true);
			assert.equal(all.override?.scope, "user");

			const settings = readJson(userSettingsPath()) as { subagents: { agentOverrides: { delegate: { disabled: boolean } } } };
			assert.equal(settings.subagents.agentOverrides.delegate.disabled, true);
		});
		...
		it("merges disabled into an existing override without dropping other fields", () => {
			...
			handleManagementAction("disable", { agent: "delegate" }, ctx);

			const settings = readJson(userSettingsPath()) as { subagents: { agentOverrides: { delegate: { model: string; disabled: boolean } } } };
			assert.deepEqual(settings.subagents.agentOverrides.delegate, { model: "openai/gpt-5.4", disabled: true });
		});
		...
		it("hides a disabled agent from agent-facing list output", () => {
			...
			assert.doesNotMatch(text, /^- delegate /m);
		});

		it("writes a project-scoped override when agentScope is project", () => {
			fs.mkdirSync(path.join(tempDir, ".pi"), { recursive: true });
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			const disabled = handleManagementAction("disable", { agent: "delegate", agentScope: "project" }, ctx);
			assert.equal(disabled.isError, false);
	...
	describe("enable", () => {
		it("restores a previously disabled builtin to runtime discovery", () => {
			...
			const enabled = handleManagementAction("enable", { agent: "delegate" }, ctx);
			assert.equal(enabled.isError, false);
			assert.match(readText(enabled), /Enabled agent 'delegate'/);
			assert.ok(discoverAgents(tempDir, "both").agents.find((a) => a.name === "delegate"));

			const settings = readJson(userSettingsPath()) as { subagents?: { agentOverrides?: Record<string, unknown> } };
			assert.equal(settings.subagents?.agentOverrides?.delegate, undefined);
		});

		it("preserves other override fields when removing the disabled flag", () => {
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			writeJson(userSettingsPath(), {
				subagents: { agentOverrides: { delegate: { model: "openai/gpt-5.4", disabled: true } } },
			});

			handleManagementAction("enable", { agent: "delegate" }, ctx);

			const settings = readJson(userSettingsPath()) as { subagents: { agentOverrides: { delegate: { model: string; disabled?: boolean } } } };
			assert.deepEqual(settings.subagents.agentOverrides.delegate, { model: "openai/gpt-5.4" });
			assert.ok(discoverAgents(tempDir, "both").agents.find((a) => a.name === "delegate"));
		});
		...
		it("points to the disabling scope when enabling the wrong scope", () => {
			fs.mkdirSync(path.join(tempDir, ".pi"), { recursive: true });
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			handleManagementAction("disable", { agent: "delegate", agentScope: "project" }, ctx);
	...
	describe("reset", () => {
		it("deletes a custom shadow file and restores the bundled builtin", () => {
			const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
			handleManagementAction("eject", { agent: "delegate" }, ctx);
			assert.equal(discoverAgents(tempDir, "both").agents.find((a) => a.name === "delegate")?.source, "user");

			const reset = handleManagementAction("reset", { agent: "delegate" }, ctx);
			assert.equal(reset.isError, false);
			assert.match(readText(reset), /Deleted custom user agent file/);
			assert.match(readText(reset), /Reset agent 'delegate' to its bundled builtin default/);
			assert.equal(fs.existsSync(userAgentPath("delegate")), false);
			assert.equal(discoverAgents(tempDir, "both").agents.find((a) => a.name === "delegate")?.source, "builtin");
		});
