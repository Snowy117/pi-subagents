...
import type { AsyncResultPayload, AsyncStatusPayload } from "../support/async-execution-harness.ts";

describe("async execution utilities — control & live streaming", { skip: !available ? "pi packages not available" : undefined }, () => {
	let tempDir: string;
	let mockPi: MockPi;
	...
	it("background event logs drop noisy message updates and cap child diagnostics", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		try {
			...
			assert.match(eventsText, /"droppedEventType":"[^"]+"/);
		} finally {
	...
	it("background runs stream child events and live output while active", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		while (Date.now() < liveDeadline && (!sawChildEvent || !sawLiveOutput)) {
			if (fs.existsSync(eventsPath)) {
				...
				sawChildEvent = content.includes('"type":"tool_execution_start"') && content.includes('"subagentSource":"child"');
			}
